...
#objetos que utilizaremos para representar para a construcao da linguagem
...
class Greet:
    def __init__(self, typ):
        #sendo {bom_dia','boa_tarde' ou 'boa_noite'}
        self.typ = typ
...
#lexer
#definicao de tokens para nossa linguagem
...
precedence = (          #precedencia para as operacoes
    ('right', 'ASSIGN'),
    ('right', 'UMINUS'),
...
# Saudaçoes para o incio do codigo
...
#interprete
#para nalisadar as operacoes feitas na arvore
    ...
    if isinstance(stmt, Greet):
        ...
        if stmt.typ == 'bom_dia':
            if 5 <= h < 12:
                ...
                print(f"E sei que o dia tem 24h, mas isso nao te desculpa de ser mal educado. Hora atual: {agora:%H:%M}")
                sys.exit(1)
        elif stmt.typ == 'boa_tarde':
            if 12 <= h < 18:
                print("Boa tarde, está coerente com o horário.")
            else:
                print(f"Parece de tarde pra voce? Hora atual: {agora:%H:%M}")
                sys.exit(1)
        elif stmt.typ == 'boa_noite':
            if (18 <= h <= 23) or (0 <= h < 5):
                print("Boa noite, está coerente com o horário.")
            else:
                print(f"Nem perto da hora de dormir. Hora atual: {agora:%H:%M}")
                sys.exit(1)
        return
    raise RuntimeError(f"Statement desconhecido: {stmt!r}")

#
#main para controlar quando o programa roda e como ele vai rodar, lendo um txt ou executando no terminal
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        # execucao do arquivo teste programa.txt
        filename = sys.argv[1]
        ...
        # caso nao rode o codigo ele executa no propio terminal
